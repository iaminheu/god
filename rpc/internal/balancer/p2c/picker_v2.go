...
func (p *v2P2cPicker) Pick(info balancer.PickInfo) (balancer.PickResult, error) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *v2P2cPicker) choose(c1, c2 *subConn) *subConn {
...
func (p *v2P2cPicker) buildDoneFunc(conn *subConn) func(doneInfo balancer.DoneInfo) {
	startTime := int64(timex.Now())
